...
def send_email_with_dataframe(df, recipient_email, sender_email, sender_password):
    # Create the email content
    day = datetime.now().strftime("%d.%m.%Y")
    subject = f"Game Results - Summerlympics ({day})" 
    date = datetime.now().strftime("%d.%m.%Y %H:%M")
    body = f"Hi,\n\nAttached are the game results from the Summerlympics app as of {date}.\n\nBest regards,\nYour Summerlyimpics App (https://summerlympics.streamlit.app/)"
    ...
    # Attach the DataFrame CSV
    part = MIMEBase('application', 'octet-stream')
    part.set_payload(csv_buffer.getvalue().encode())
    encoders.encode_base64(part)
    part.add_header('Content-Disposition', 'attachment; filename=game_results.csv')
    msg.attach(part)
...
applause_placeholder = st.markdown("", unsafe_allow_html=True)

def play_applause(file_path: str):
    """
    ...
    audio_html = f"""
        ...
        <script type="text/javascript">
            // Set volume to 70% once the element is available
    ...
    applause_placeholder.markdown(audio_html, unsafe_allow_html=True)

commentator_placeholder = st.markdown("", unsafe_allow_html=True)

def play_commentator(file_path: str):
    """
    Reads a local audio file and injects an HTML <audio> tag that autoplays the sound.
    """
    # Ensure the file exists
    ...
    commentator_placeholder.markdown(audio_html, unsafe_allow_html=True)
...
def select_commentator_voice(winner, game_number):
    ...
    close_game = abs(p1_score - p2_score) <= 2 and p1_score + p2_score >= 10
...
def main():
    ...
    if (st.session_state.p1_score >= st.session_state.target_score) or (st.session_state.p2_score >= st.session_state.target_score):
        ...
        # Compute dynamic minimum of games
    ...
    if st.session_state.celebration:
        if st.session_state.celebration == 'p1':
            ...
            if st.session_state.p1_score >= st.session_state.target_score:
                ...
                play_applause(applause_file)
                commentator_file = select_commentator_voice('p1', st.session_state.current_game)
                play_commentator(commentator_file)
        else:
            ...
            if st.session_state.p2_score >= st.session_state.target_score:
                ...
                applause_file = f"sounds/applause_{random.randint(1,5)}.mp3"
                play_applause(applause_file)
                commentator_file = select_commentator_voice('p2', st.session_state.current_game)
                play_commentator(commentator_file)
        st.session_state.celebration = None
    ...
    if st.session_state.is_tie:
        ...
        st.markdown(
            ...
            <div class="center-warning">
                ...
                <br>
                You can change the game in the sidebar. Good luck!
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    if st.session_state.game_over and not st.session_state.game_over_handled:
        ...
        # Send email
        recipient_email = "konrad.schulte3@gmx.de"
        sender_email = st.secrets["email"]
        sender_password = st.secrets["password"]
        send_email_with_dataframe(df, recipient_email, sender_email, sender_password)
